            ...
            <TabContext value={value}>
                <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                    <TabList textColor="secondary"
                        indicatorColor="secondary" onChange={handleChange} aria-label="params tabs">
                        <Tab sx={{ textTransform: 'none' }} label="useParams" value="1" />
                        <Tab sx={{ textTransform: 'none' }} label="searchParams" value="2" />
                        <Tab sx={{ textTransform: 'none' }} label="locationState" value="3" />
                    </TabList>
                </Box>
                <TabPanel value="1">{JSON.stringify(params)}</TabPanel>
                <TabPanel value="2">
                    <Typography variant='subtitle2' className=" text-gray-500"> [searchParams] = useSearchParams()</Typography >
                    <Typography variant='subtitle2' className=" text-gray-500">
                        searchParams.get(key)</Typography >
...
function GlobalLoading() {
    ...
    return (<div >
        <Dialog open={state} PaperProps={{ sx: { borderRadius: "12px" }, className: "shadow-md" }}>
...
export default function PersistentDrawerLeft() {
    ...
    const getNavLinkClassName = ({ isActive, isPending }: { isActive: boolean, isPending: boolean }) => {
        let data = isActive ? "bg-gray-500 text-white" : isPending ? "pending" : "bg-gray-100 text-black"
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            ...
            <Modal
                ...
                onClose={() => { setOpenDebugSwitch(false) }}
